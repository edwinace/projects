...
public class MainControllerTest {
	...
	public void getLayoutTest() throws Exception {
		mockMvc.perform(get(UriResources.Main.LAYOUT)).andExpect(status().is2xxSuccessful()).andExpect(status().isOk())
				.andExpect(content().string(containsString(ServiceMessages.Program.ORGANIZATION_NAME)))
				.andExpect(content().string(containsString(ServiceMessages.Program.PROGRAM_NAME)));
	}
}
