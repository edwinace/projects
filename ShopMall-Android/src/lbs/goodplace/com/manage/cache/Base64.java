...
public class Base64 {
	...
	public static void main(String[] args) throws Exception {
		...
		String s = encodeString("中国你好aaaa_#123$5~ 	E", "gbk");
	...
	public static class InputStream extends java.io.FilterInputStream {
		private boolean mEncode; // Encoding or decoding
		private int mPosition; // Current position in the buffer
		private byte[] mBuffer; // Small buffer holding converted data
		private int mBufferLength; // Length of buffer (3 or 4)
		private int mNumSigBytes; // Number of meaningful bytes in the buffer
		private int mLineLength;
		private boolean mBreakLines; // Break lines at less than 80 characters
		...
		public InputStream(java.io.InputStream in, boolean encode, boolean breakLines) {
			super(in);
			this.mBreakLines = breakLines;
			this.mEncode = encode;
			this.mBufferLength = encode ? 4 : 3;
			this.mBuffer = new byte[mBufferLength];
			this.mPosition = -1;
			this.mLineLength = 0;
		} // end constructor
		...
		public int read() throws java.io.IOException {
			...
			if (mPosition < 0) {
				if (mEncode) {
					...
					if (numBinaryBytes > 0) {
						encode3to4(b3, 0, numBinaryBytes, mBuffer, 0);
						mPosition = 0;
						mNumSigBytes = 4;
					} // end if: got data
					else {
				...
				// Else decoding
					...
					if (i == 4) {
						mNumSigBytes = decode4to3(b4, 0, mBuffer, 0);
						mPosition = 0;
			...
			if (mPosition >= 0) {
				...
				if (/* !encode && */mPosition >= mNumSigBytes) {
					return -1;
				}

				if (mEncode && mBreakLines && mLineLength >= MAX_LINE_LENGTH) {
					mLineLength = 0;
					return '\n';
				} // end if
				else {
					mLineLength++; // This isn't important when decoding
									// but throwing an extra "if" seems
									// just as wasteful.

					int b = mBuffer[mPosition++];

					if (mPosition >= mBufferLength) {
						mPosition = -1;
					}
	...
	public static class OutputStream extends java.io.FilterOutputStream {
		...
		public void write(int theByte) throws java.io.IOException {
			if (mEncode) {
				mBuffer[mPosition++] = (byte) theByte;
				if (mPosition >= mBufferLength) // Enough to encode.
				{
					out.write(Base64.encode3to4(mBuffer, mBufferLength));

					mLineLength += 4;
					if (mBreakLines && mLineLength >= MAX_LINE_LENGTH) {
						out.write(NEW_LINE);
						mLineLength = 0;
					} // end if: end of line
			...
			// Else, Decoding
				...
				if (DECODABET[theByte & 0x7f] > WHITE_SPACE_ENC) {
					mBuffer[mPosition++] = (byte) theByte;
					if (mPosition >= mBufferLength) // Enough to output.
					{
						out.write(Base64.decode4to3(mBuffer));
						mPosition = 0;
					} // end if: enough to output
		...
		public void flush() throws java.io.IOException {
			...
			if (mPosition > 0) {
				if (mEncode) {
					out.write(Base64.encode3to4(mBuffer, mPosition));
				} // end if: encoding
				else {
